...
function renderRecords() {
  ...
  for (let i = 0; i < records.length; i++) {
    ...
    const tdNum = createEl('td', { text: String(i + 1) });
    const tdScore = createEl('td', { text: String(records[i]) });
...
function addNewTiles(field) {
  ...
  for (let k = 0; k < howMany && k < empties.length; k++) {
    let cell = empties[k];
    field[cell.i][cell.j] = Math.random() < 0.9 ? 2 : 4;
  }
}
...
function showGameOver() {
  ...
  if (!overlay) {
    overlay = createEl('div', { className: 'game-over', text: 'Нет возможных ходов. Игра окончена!' });
    containerHeader.appendChild(overlay);
  }
}
...
function undo() {
  ...
  field = prevField.map(function(row) {
    return row.slice();
  });
...
function down() {
  ...
  for (let col = 0; col < 4; col++) {
    ...
    for (let row = 3, i = 0; row >= 0; row--, i++) {
      if (field[row][col] !== column[i]) {
        field[row][col] = column[i];
        moved = true;
      }
    }
  }
...
function up() {
  ...
  for (let col = 0; col < 4; col++) {
    ...
    for (let row = 0; row < 4; row++) {
      if (field[row][col] !== column[row]) {
        field[row][col] = column[row];
        moved = true;
      }
    }
  }
...
function handleMove(direction) {
  ...
  const beforeMove = field.map(function(row) {
    return row.slice();
  });
